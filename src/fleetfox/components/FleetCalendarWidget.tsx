...
export default function FleetCalendarWidget({ events, height, onEventClick }: FleetCalendarWidgetProps) {
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.45rem', flex: 1, minHeight: 0 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div style={{ display: 'grid', gap: '0.12rem' }}>
            <div style={{ color: '#d4380d', fontWeight: 700, fontSize: '1.14rem', lineHeight: 1.1 }}>{t('fleetfox.calendar.title')}</div>
        ...
        <div style={{ display: 'grid', gap: '0.25rem', fontSize: '0.62rem', lineHeight: 1.2, marginTop: '0.45rem', minHeight: 0, overflowY: 'auto' }}>
          {upcomingEvents.length === 0 ? (
            <span style={{ color: '#64748b' }}>{t('fleetfox.calendar.empty')}</span>
          ) : (
