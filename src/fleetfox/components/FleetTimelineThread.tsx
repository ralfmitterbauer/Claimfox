...
type FleetTimelineThreadProps = {
  events: TimelineEvent[]
  emptyLabel: string
}

export default function FleetTimelineThread({ events, emptyLabel }: FleetTimelineThreadProps) {
  if (events.length === 0) {
      ...
      {events.map((event) => (
        <div key={event.id} style={{ borderBottom: '1px solid #e2e8f0', paddingBottom: '0.6rem' }}>
          ...
          <div style={{ fontSize: '0.8rem', color: '#64748b' }}>
            {new Date(event.createdAt).toLocaleString()} · {event.meta?.actor ?? 'fleet-ops'}
          </div>
