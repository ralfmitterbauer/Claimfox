...
export default function LegalIntakePage() {
  ...
  const copy = lang === 'en'
    ? {
        title: 'Legal Intake — Legal Dashboard',
        subtitle: 'Intake triage, completeness checks and SLA adherence for legal requests.',
        lastUpdated: 'Last updated: 27 Jan 2026, 14:40',
        kpi: {
        ...
        actions: {
          request: 'Request additional documents',
          assign: 'Assign to Claims Legal',
          escalate: 'Escalate to Senior Legal'
        },
        slaPanel: {
    ...
    : {
        title: 'Legal Intake — Legal Dashboard',
        subtitle: 'Intake-Triage, Vollständigkeitsprüfung und SLA-Einhaltung für Legal-Anfragen.',
        lastUpdated: 'Zuletzt aktualisiert: 27. Jan 2026, 14:40',
        kpi: {
          ...
          completeness: 'Vollständigkeitsquote'
        },
        table: {
          title: 'Decision Inbox',
          id: 'Case ID',
        ...
        actions: {
          request: 'Zusätzliche Dokumente anfordern',
          assign: 'An Claims Legal zuweisen',
          escalate: 'An Senior Legal eskalieren'
        },
        slaPanel: {
          title: 'SLA & Governance',
          buckets: 'SLA Buckets',
  ...
  const cases: LegalCase[] = [
    {
      id: 'LI-2041',
      type: 'Claim',
      trigger: 'Missing loss documentation',
      severity: 'Medium',
      slaDue: '2026-01-27T16:00:00Z',
      slaBucket: 'due_today',
      recommendation: 'Request additional documents',
      status: 'In review',
      applicableRules: ['Binder §4.2 Documentation', 'Claims SOP v2.3'],
      keyRisks: ['Incomplete evidence chain', 'SLA breach risk'],
      evidenceQuality: 62,
      escalationRequired: false,
      summary: 'Initial FNOL submitted with partial documentation; missing signed loss statement.'
    },
    {
      id: 'LI-2047',
      type: 'Regulatory',
      trigger: 'Delegated authority scope check',
      severity: 'High',
      slaDue: '2026-01-28T12:30:00Z',
      slaBucket: 'due_48h',
      recommendation: 'Escalate to Senior Legal',
      status: 'Open',
      applicableRules: ['Delegated Authority Schedule 2025', 'Lloyd’s Minimum Standards MS14'],
      keyRisks: ['Out-of-scope authority', 'Carrier approval required'],
      evidenceQuality: 71,
      escalationRequired: true,
      summary: 'Request touches a class not in current delegated authority schedule.'
    },
    {
      id: 'LI-2052',
      type: 'Wording',
      ...
      severity: 'Low',
      slaDue: '2026-01-27T10:00:00Z',
      slaBucket: 'breached',
      recommendation: 'Assign to Claims Legal',
      status: 'Breached',
      applicableRules: ['Wording Control v1.9', 'Deviation Policy v3.1'],
      keyRisks: ['Unapproved deviation', 'Documentation gap'],
      evidenceQuality: 54,
      escalationRequired: true,
      summary: 'Broker requested deviation without impact analysis attachments.'
    }
  ]

  const logs: LogEntry[] = [
    { timestamp: '2026-01-27 09:12', role: 'Legal Intake', action: 'Case created', ruleVersion: 'v3.1', evidenceStatus: 'missing', note: 'Awaiting documents' },
    { timestamp: '2026-01-27 08:40', role: 'Legal Intake', action: 'SLA flagged', ruleVersion: 'v3.1', evidenceStatus: 'stale', note: 'Due today' },
    { timestamp: '2026-01-26 17:05', role: 'Senior Legal', action: 'Scope review', ruleVersion: 'v3.0', evidenceStatus: 'ok', note: 'Escalation pending' },
    { timestamp: '2026-01-26 11:28', role: 'Legal Intake', action: 'Evidence request', ruleVersion: 'v3.1', evidenceStatus: 'missing', note: 'Loss statement' },
    { timestamp: '2026-01-25 16:52', role: 'Compliance', action: 'Audit check', ruleVersion: 'v2.9', evidenceStatus: 'ok', note: 'No issues' }
  ]
  ...
  const kpis = React.useMemo(() => {
    ...
    const completeness = Math.round(cases.reduce((acc, item) => acc + item.evidenceQuality, 0) / cases.length)
    return {
      open,
      dueToday,
      breaches,
    ...
    <section className="uw-page">
      ...
      <div className="uw-container">
        <div className="uw-grid uw-kpi">
          <Card title={copy.kpi.open} variant="glass" className="uw-card"><div className="uw-card-body"><strong>{kpis.open}</strong><MiniBars data={[12, 14, 13, 15, 16]} /></div></Card>
          ...
          <Card title={copy.kpi.breaches} variant="glass" className="uw-card"><div className="uw-card-body"><strong>{kpis.breaches}</strong><MiniBars data={[2, 2, 3, 3, 2]} /></div></Card>
          ...
          <Card title={copy.kpi.completeness} variant="glass" className="uw-card"><div className="uw-card-body"><strong>{kpis.completeness}</strong><MiniBars data={[78, 80, 82, 84, 86]} /></div></Card>
        </div>
          ...
          <Card variant="glass" className="uw-card">
            <div className="uw-card-body" style={{ gap: '0.75rem' }}>
              ...
              <div style={{ overflowX: 'auto' }}>
                <table className="uw-table" aria-label="Legal intake decision inbox">
                  <thead>
                    <tr>
                      <th>{copy.table.id}</th>
            ...
            <div className="uw-card-body" style={{ gap: '0.75rem' }}>
              ...
              <div className="uw-panel">
                ...
                <div className="uw-actions">
                  <Button variant="secondary" onClick={() => {}}>{copy.actions.request}</Button>
                  <Button variant="secondary" onClick={() => {}}>{copy.actions.assign}</Button>
        ...
        <div className="uw-grid uw-triplet">
          ...
          <Card variant="glass" className="uw-card">
            <div className="uw-card-body">
              ...
              <div style={{ overflowX: 'auto' }}>
                <table className="uw-table" aria-label="Legal intake audit log">
                  <thead>
                    <tr>
